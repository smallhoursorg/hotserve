...
caddy-hotswap folds that entire loop into the reverse proxy that was
...
| Nomad-era piece | caddy-hotswap replacement |
|---|---|
...
| `nomadService` template + SIGUSR1 | `http.reverse_proxy.upstreams.hotswap` atomic port swap |
...
- The webhook MUST authenticate every request with a constant-time
  comparison of `X-Hotswap-Secret`, and MUST NOT reveal whether an app
...
Three cooperating modules, flat package `hotswap`:

1. **`hotswap`** (`hotswap.go`) — a `caddy.App`, registered as a
...
2. **`http.handlers.hotswap_webhook`** (`handler.go`) — resolves the
   app module via `ctx.App("hotswap")`; authenticates; `POST /<app>`
...
3. **`http.reverse_proxy.upstreams.hotswap`** (`upstreams.go`) —
...
keyed by app name (`hotswap.go`). Provision takes a pool reference;
...
| File | Concern |
|---|---|
| `hotswap.go` | App module, config structs + defaults, Validate, pool wiring |
...
3. **Admin endpoints** (`caddy hotswap status/rollback`) alongside the
