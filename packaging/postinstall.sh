...
EOF
	...
	# had as its children.
	# AppArmorProfile: the app sandbox is built on user namespaces,
	# which Ubuntu 24.04+ refuses to unconfined processes; the shipped
	# profile grants exactly that to exactly this manager. "-" ignores
	# a missing profile (Debian, or AppArmor off), so the manager still
	# starts there — the sandbox probe then reports what the host can do.
	mkdir -p "/etc/systemd/system/user@$uid.service.d"
	cat > "/etc/systemd/system/user@$uid.service.d/10-hotserve.conf" <<EOF
# Generated by the hotserve package (postinstall); do not edit.
[Service]
LimitNOFILE=1048576
AppArmorProfile=-hotserve-user-manager
EOF
