...
func init() {
	httpcaddyfile.RegisterGlobalOption("hotswap", parseGlobalOption)
	httpcaddyfile.RegisterHandlerDirective("hotswap_webhook", parseWebhookDirective)
	// The webhook is terminal, but give it a sane default position for
	// site blocks that mix it with other directives.
	httpcaddyfile.RegisterDirectiveOrder("hotswap_webhook", httpcaddyfile.Before, "reverse_proxy")
}
...
func (h *Handler) UnmarshalCaddyfile(d *caddyfile.Dispenser) error {
	...
	if d.NextBlock(0) {
		return d.Err("hotswap_webhook takes no block")
	}
	return nil
}
...
func (u *Upstreams) UnmarshalCaddyfile(d *caddyfile.Dispenser) error {
	...
	if d.NextBlock(0) {
		return d.Err("dynamic hotswap takes no block")
	}
	return nil
}
