...
deploy() {
	curl -s -o /tmp/deploy-body -w '%{http_code}' --max-time 60 \
		-X POST -H "X-Hotswap-Secret: $SECRET" \
...
until curl -fs -o /dev/null "$ART/demo-v1.tar.gz" \
	&& [ "$(curl -s -o /dev/null -w '%{http_code}' -H "X-Hotswap-Secret: $SECRET" "$HOOK")" = "200" ]; do
	i=$((i + 1))
	if [ "$i" -ge 60 ]; then
...
c=$(curl -s -o /dev/null -w '%{http_code}' -X POST -H "X-Hotswap-Secret: wrong" -d '{}' "$HOOK")
...
s=$(curl -s -H "X-Hotswap-Secret: $SECRET" "$HOOK")
case "$s" in
