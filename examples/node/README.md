...
(`journalctl -u hotserve`) names the check that refused it, within
the box's budget for failed authentications: ten a minute from one
address (past that, 429) and a hundred a minute in all (past that,
401), neither written until the minute passes.
