...
**Your reverse proxy is your deploy pipeline.** caddy-hotswap turns a
...
| `nomadService` template re-render + SIGUSR1 | `dynamic hotswap <app>` |
...
```sh
xcaddy build --with github.com/hotsauce-team/caddy-hotswap
```
...
RUN xcaddy build v2.11.4 --with github.com/hotsauce-team/caddy-hotswap
...
{
	hotswap {
		# root /var/lib/hotswap                # where releases/state live (default)
		webhook_secret {env.HOTSWAP_SECRET}    # default secret for all apps
		...
		app blog {                             # a Node.js app
			...
			env_file /etc/hotswap/blog.env     # optional KEY=VALUE file
...
|---|---|---|
| `root` | `/var/lib/hotswap` | Releases, shared data, state per app |
...
`X-Hotswap-Secret: <secret>` and JSON body:
...
- name: Deploy
  run: |
    curl --fail --max-time 600 -X POST \
      -H "X-Hotswap-Secret: ${{ secrets.HOTSWAP_SECRET }}" \
      -d '{
...
deploy:
  ...
  script:
    ...
    - |
      curl --fail --max-time 600 -X POST \
        -H "X-Hotswap-Secret: $HOTSWAP_SECRET" \
        -d "{
...
- **Apps are child processes of Caddy.** Config reloads never touch
  ...
  hotswap relaunches each app's recorded current version and serves it
