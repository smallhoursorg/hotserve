...
const secretHeader = "X-Hotswap-Secret"

// maxPayloadBytes bounds the webhook JSON body.
const maxPayloadBytes = 64 * 1024

// Handler implements the hotswap webhook endpoint. Mount it in its own
...
func (Handler) CaddyModule() caddy.ModuleInfo {
	return caddy.ModuleInfo{
		ID:  "http.handlers.hotswap_webhook",
		New: func() caddy.Module { return new(Handler) },
	}
}

// Provision binds the handler to the hotswap app.
func (h *Handler) Provision(ctx caddy.Context) error {
	...
	h.app = appModule.(*App)
	if len(h.app.Apps) == 0 {
		return fmt.Errorf("hotswap_webhook is configured but no apps are defined in the hotswap global options")
	}
	return nil
}
